...
# Alternative is the following line, as player_database is joined to penalty data by player's number (instead of name)
...
totals_from_results = totals_winners_from_results %>%
  ...
  left_join(player_database, by = c("Year" = "years",
                                    "Team" = "squad",
                                    "GK_no" = "no")) %>%
  select(Shootout_no:caps, captain, dob_date:age_int, club_current_name)
...
penalty_shootout_details_full = penalty_shootout_details_raw %>%
  ...
  left_join(player_database, by = c("Year" = "years",
                                    ...
                                    #"Player_name" = "player",
                                    "Number" = "no")) %>%
  select(Shootout_no:Count_kicks_opponent, Winner:club_url, captain, dob_date:age_int, nat_team_alpha3, nat_team_confederation:nat_team_current_name,
  ...
  ungroup() %>%
  mutate(date_match = as.Date(Date.x, format = "%d/%m/%Y"),
         player_date_dob = as.Date(dob_date, format = "%Y-%m-%d"),
         year_adj = ifelse(month(player_date_dob) * 100 + day(player_date_dob) > month(date_match) * 100 + day(date_match), -1, 0),
         day_adj = ifelse(month(player_date_dob) == 2 & day(player_date_dob) == 29, -1, 0),
         date_last_bday = make_date(year(date_match) + year_adj, month(player_date_dob), day(player_date_dob) + day_adj),
         age_last_bday = year(date_last_bday) - year(player_date_dob),
         days_since_last_bday = as.numeric(date_match - date_last_bday),
         age_at_date_match = age_last_bday + days_since_last_bday/365.25)
...
success_by_vars(penalty_shootout_details_full, age_int)       # age at start of tournament
success_by_vars(penalty_shootout_details_full, age_last_bday) # age on day of match
...
youngest_pen_taker = penalty_shootout_details_full %>%
  arrange(age_at_date_match) %>%
  select(Player_name:Opponent, Year, age_last_bday, days_since_last_bday, pen_result)
head(youngest_pen_taker, 10)

oldest_pen_taker = penalty_shootout_details_full %>%
  arrange(desc(age_at_date_match)) %>%
  select(Player_name:Opponent, Year, age_last_bday, days_since_last_bday, pen_result)
head(oldest_pen_taker, 10)
